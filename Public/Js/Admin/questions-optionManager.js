optionManager = {
	...
	disableOptAgainestCase: function(type){
		...
		switch(type){
			...
			case 'checkbox':
							...
							break;
			case 'text':
							$options.not('[type=text]').attr('disabled', 'disabled').unbind('click');
							break;
	...
	collectOptions: function(){
		...
		$('#question-form .option-list .option-item').each(function(i){
			...
			var text = (type!='radio' && type!='checkbox')? $this.find('textarea.form-control').val():$this.find('input.form-control').val();

			
			options[i+1] = {'type':type, 'text':text};
...
OptionTemplate = {
	...
	get: function(type){
		...
		switch(type){
			...
			case 'text': //简答项
					return '<div class="option-item text_option"><div> <textarea class="form-control"> </textarea> <img class="option-del" src="/Public/Images/btn-del.png" title="删除该选项"></div> </div>';
