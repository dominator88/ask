...
class ReplyController extends CommonController
{
    ...
    public function delete()
    {
        ...
        if( $state === false ){
            ...
            $this->success('回答记录删除成功', U('Admin/Reply/all'));
        }
    }
	
	//指定答案的试卷
	public function info(){
		$questionnaireId = I('get.questionnaireid');
		$replyid = I('get.rid');
		$m = D('Admin/Questionnaires');
		$mr= M('Reply');
		$data = $m->getQuestion($questionnaireId);
		$where['id'] = array('EQ',$replyid);
		$answer = json_decode($mr->where($where)->getField('reply'),true);
		$userid = $mr->where($where)->getField('userid');
...
//		print_r($indexarr);
		...
		$user['createtime'] = $mr->where($where)->getField('createtime');
		$content = M("Replyvalidata")->where('replyid='.$replyid)->getField('content');
		
		$remarks = M("Replyremark")->where('replyid='.$replyid)->getField('content');

		$this->assign('remarks',htmlspecialchars_decode($remarks));

		
		$this->assign("validatacontent",htmlspecialchars_decode($content));
	...
	public function getReplyInfo(){
		...
		$data['now'] = $m->getInfo($questionid,$userid,'now');
		$data['now'] = json_decode($data['now']['reply'],true);
		$data['last'] = $m->getInfo($questionid,$userid,'last');
		$data['last'] = json_decode($data['last']['reply'],true);
		$data['before'] = $m->getInfo($questionid,$userid,'before');
		$data['before'] = json_decode($data['last']['before'],true);
		return $data;
	}
}